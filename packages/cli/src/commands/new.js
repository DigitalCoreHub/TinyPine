...
export function newCommand(program) {
    program
        ...
        .option(
            "-t, --template <template>",
            "Specify template (vanilla, tailwind, spa, ssr, ui-ready)"
        )
        .option(
            "-f, --features <features>",
            "Comma-separated features (router,i18n,ui,devtools)"
        )
        ...
        .action(async (projectName, options) => {
            ...
            try {
                ...
                if (features.length > 0) {
                    ...
                    spinner.succeed(
                        `Installed features: ${features.join(", ")}`
                    );
                }
                ...
                console.log(
                    chalk.cyan(
                        `📦 Created TinyPine project in ./${projectName}`
                    )
                );
                console.log(
                    chalk.cyan(
