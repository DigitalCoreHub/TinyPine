...
describe("TinyPine Core - Directives", () => {
    ...
    it("should toggle t-show directive", async () => {
        document.body.innerHTML = `
    ...
    `;
        ...
        if (typeof window.TinyPine !== "undefined") {
            ...
            // Use reactive proxy to trigger updates
            const scope = document.querySelector("[t-data]");
            scope._tinypineContext.data.visible = false;

            // Wait for debounced update (use real timers, it's only 20ms)
...
describe("TinyPine Performance", () => {
    it("should initialize quickly", () => {
        ...
        document.body.innerHTML =
            '<div t-data="{ count: 0 }"><span t-text="count"></span></div>';
